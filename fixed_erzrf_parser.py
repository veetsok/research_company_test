#!/usr/bin/env python3
"""
ИСПРАВЛЕННЫЙ парсер для сайта erzrf.ru - полный парсинг всех компаний
...
class FixedERZRFParser:
    ...
    def extract_company_data(self, company_url, attempt=1):
        ...
        try:
            ...
            r.html.render(timeout=30, wait=8, sleep=3)
                ...
                if title:
                    ...
                    if title_text and ' - ' in title_text:
                        ...
                        if name and len(name) < 200 and 'ЕРЗ' not in name:
                            company_data['название'] = self.clean_text(name)
                            title_found = True
            except:
                pass
                    ...
                    try:
                        ...
                        if elements:
                            ...
                            if title_text and len(title_text) < 200 and 'ЕРЗ' not in title_text:
                                company_data['название'] = self.clean_text(title_text)
                                title_found = True
                                break
                    except:
                        continue
            ...
            # Способ 1: Ищем в HTML коде специальные блоки
            try:
                ...
                for pattern in site_patterns:
                    ...
                    for match in matches:
                        if (match and 'http' in match and 'erzrf.ru' not in match and 
                            'metrika.yandex' not in match and 'uniteddevelopers' not in match and
                            'google.com' not in match and 'support.google' not in match):
            ...
            # Способ 2: Ищем среди всех ссылок с фильтрацией
            if not site_found:
                try:
                    ...
                    # Ищем ссылки, которые похожи на сайты компаний
                    for link in all_links:
                        href = link.attrs.get('href', '')
                        if (href and 'http' in href and 
                            not any(domain in href.lower() for domain in excluded_domains) and
                            any(tld in href for tld in ['.ru', '.com', '.рф', '.org', '.net']) and
                            len(href) < 100):  # Исключаем очень длинные URL
            ...
            try:
                # Ищем в HTML коде специальные блоки для соцсетей
                html_content = r.html.html
                
                # Паттерны для поиска соцсетей
                social_patterns = [
                    r'<app-org-social[^>]*>.*?href="([^"]*(?:vk\.com|facebook\.com|instagram\.com|t\.me|youtube\.com|ok\.ru)[^"]*)"',
                    r'<div[^>]*id="org-social"[^>]*>.*?href="([^"]*(?:vk\.com|facebook\.com|instagram\.com|t\.me|youtube\.com|ok\.ru)[^"]*)"',
                    r'Соц\. сети.*?href="([^"]*(?:vk\.com|facebook\.com|instagram\.com|t\.me|youtube\.com|ok\.ru)[^"]*)"'
                ]
                
                for pattern in social_patterns:
                    matches = re.findall(pattern, html_content, re.DOTALL | re.IGNORECASE)
                    for match in matches:
                        if (match and 'erzrf_ru' not in match and '+kf4d9SlCRpRiMGQy' not in match):
            ...
            # Извлечение города - ИСПРАВЛЕННАЯ логика
            city_found = False
            ...
            # Ищем в тексте страницы с улучшенными паттернами
            city_patterns = [
                r'г\.?\s*([А-ЯЁ][а-яё\-]{2,20})',  # г. Москва
                r'город\s+([А-ЯЁ][а-яё\-]{2,20})',  # город Москва
                r'([А-ЯЁ][а-яё\-]{2,20})\s+область',  # Московская область
                r'([А-ЯЁ][а-яё\-]{2,20})\s+край',  # Краснодарский край
                r'([А-ЯЁ][а-яё\-]{2,20})\s+республика'  # Татарстан республика
            ]
            
            for pattern in city_patterns:
                matches = re.findall(pattern, page_text, re.IGNORECASE)
                for match in matches:
                    city = self.clean_text(match)
                    # Фильтруем плохие совпадения
                    if (len(city) >= 3 and len(city) <= 20 and 
                        not any(word in city.lower() for word in [
                            'застройщик', 'компания', 'группа', 'строительство', 
                            'инвест', 'девелопмент', 'проект', 'учредитель',
                            'алтайский', 'амурская', 'архангельская', 'астраханская',
                            'белгородская', 'брянская', 'владимирская', 'волгоградская',
                            'вологодская', 'воронежская', 'донецкая', 'еврейская',
                            'забайкальский', 'запорожская', 'ивановская', 'иркутская'
                        ]) and
                        city.lower() not in ['санкт', 'петербург', 'российская', 'народная']):
            ...
            # Если город все еще не найден, ищем более простые паттерны
            if not city_found:
                simple_cities = ['Москва', 'Санкт-Петербург', 'Новосибирск', 'Екатеринбург', 
        ...
        for i, company_url in enumerate(all_company_links, 1):
            ...
            if company_data:
                ...
                if company_data['город']:
                    print(f"  🏙️ Город: {company_data['город']}")
    ...
    def save_to_excel(self, filename='erzrf_companies_fixed.xlsx'):
    ...
    def save_to_csv(self, filename='erzrf_companies_fixed.csv'):
...
def main():
    ...
    try:
        ...
        parser.save_to_excel('erzrf_companies_fixed.xlsx')
        parser.save_to_csv('erzrf_companies_fixed.csv')
        parser.save_failed_urls('failed_urls_fixed.txt')
