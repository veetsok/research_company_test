...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('erzrf_complete_parser.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class ERZRFCompleteParser:
    def __init__(self):
    ...
    def parse_top_pages(self, target_companies: int = 250) -> List[Dict]:
        ...
        while len(companies) < target_companies:
            ...
            logger.info(f"На странице {page} найдено {page_companies} компаний")
            
            # Если на странице нет компаний, значит достигли конца
            if page_companies == 0:
                logger.info("Больше компаний не найдено")
                break
            
            page += 1
            time.sleep(1)  # Задержка между страницами
        
        logger.info(f"Всего найдено {len(companies)} компаний")
        return companies
    ...
    def find_company_website(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        # Ищем блок с сайтом по структуре из примера
        website_blocks = soup.find_all('div', class_='cmn_row')
        
        for block in website_blocks:
            # Ищем элемент с названием "Сайт"
            name_element = block.find('div', class_='name')
            if name_element and 'Сайт' in name_element.get_text():
    ...
    def find_social_networks(self, soup: BeautifulSoup) -> List[str]:
        ...
        # Ищем блок с соцсетями по структуре из примера
        social_blocks = soup.find_all('div', class_='cmn_row')
        
        for block in social_blocks:
            # Ищем элемент с названием "Соц. сети"
            name_element = block.find('div', class_='name')
            if name_element and 'Соц. сети' in name_element.get_text():
                # Ищем все ссылки в этом блоке
                links = block.find_all('a', href=True)
                for link in links:
                    href = link.get('href', '')
                    if self.is_social_network(href):
    ...
    def save_to_csv(self, filename: str = 'top_250_zastroyshchiki_complete.csv'):
    ...
    def run(self, target_companies: int = 250, parse_details: bool = True):
        """Запуск парсера"""
        logger.info(f"Запуск полного парсера для топ-{target_companies} застройщиков")
        ...
        try:
            ...
            self.save_to_csv('basic_companies_list_complete.csv')
            ...
            if parse_details:
                ...
                for i, company in enumerate(self.companies_data):
                    ...
                    if (i + 1) % 25 == 0:
                        self.save_to_csv(f'partial_results_complete_{i+1}.csv')
            ...
            if self.companies_data:
                self.save_to_csv('error_backup_complete.csv')

def main():
    parser = ERZRFCompleteParser()
    ...
    try:
        ...
        if parser.companies_data:
            parser.save_to_csv('interrupted_results_complete.csv')
