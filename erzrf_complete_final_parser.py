...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('erzrf_complete_final_parser.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class ERZRFCompleteFinalParser:
    def __init__(self):
    ...
    def parse_all_top_pages(self, target_companies: int = 250) -> List[Dict]:
    ...
    def save_to_csv(self, filename: str = 'top_250_zastroyshchiki_complete_final.csv'):
    ...
    def run(self, target_companies: int = 250, parse_details: bool = True):
        """Запуск парсера"""
        logger.info(f"Запуск полного финального парсера для топ-{target_companies} застройщиков")
        
        try:
            # Получаем список компаний со всех страниц
            companies = self.parse_all_top_pages(target_companies)
            ...
            self.save_to_csv('basic_companies_list_complete_final.csv')
            ...
            if parse_details:
                ...
                for i, company in enumerate(self.companies_data):
                    ...
                    if (i + 1) % 25 == 0:
                        self.save_to_csv(f'partial_results_complete_final_{i+1}.csv')
            ...
            if self.companies_data:
                self.save_to_csv('error_backup_complete_final.csv')

def main():
    parser = ERZRFCompleteFinalParser()
    ...
    try:
        ...
        if parser.companies_data:
            parser.save_to_csv('interrupted_results_complete_final.csv')
