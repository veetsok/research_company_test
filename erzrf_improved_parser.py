...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('erzrf_improved_parser.log'),
...
class ERZRFImprovedParser:
    ...
    def find_social_networks(self, soup: BeautifulSoup) -> List[str]:
        ...
        for block in social_blocks:
            ...
            if name_element and 'Соц. сети' in name_element.get_text():
                ...
                for link in links:
                    href = link.get('href', '')
                    if self.is_social_network(href) and not self.is_erzrf_social(href):
    ...
    def save_to_csv(self, filename: str = 'top_250_zastroyshchiki_improved.csv'):
    ...
    def run(self, target_companies: int = 250, parse_details: bool = True):
        """Запуск парсера"""
        logger.info(f"Запуск улучшенного парсера для топ-{target_companies} застройщиков")
        ...
        try:
            ...
            self.save_to_csv('basic_companies_list_improved.csv')
            ...
            if parse_details:
                ...
                for i, company in enumerate(self.companies_data):
                    ...
                    if (i + 1) % 25 == 0:
                        self.save_to_csv(f'partial_results_improved_{i+1}.csv')
            ...
            if self.companies_data:
                self.save_to_csv('error_backup_improved.csv')
...
def main():
    ...
    try:
        ...
        if parser.companies_data:
            parser.save_to_csv('interrupted_results_improved.csv')
