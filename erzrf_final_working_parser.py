...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('erzrf_final_working_parser.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class ERZRFFinalWorkingParser:
    def __init__(self):
    ...
    def save_to_csv(self, filename: str = 'top_250_zastroyshchiki_final_working.csv'):
    ...
    def run(self, target_companies: int = 250, parse_details: bool = True):
        """Запуск парсера"""
        logger.info(f"Запуск финального рабочего парсера для топ-{target_companies} застройщиков")
        ...
        try:
            ...
            self.save_to_csv('basic_companies_list_final_working.csv')
            ...
            if parse_details:
                ...
                for i, company in enumerate(self.companies_data):
                    ...
                    if (i + 1) % 25 == 0:
                        self.save_to_csv(f'partial_results_final_working_{i+1}.csv')
            ...
            if self.companies_data:
                self.save_to_csv('error_backup_final_working.csv')

def main():
    parser = ERZRFFinalWorkingParser()
    ...
    try:
        ...
        if parser.companies_data:
            parser.save_to_csv('interrupted_results_final_working.csv')
